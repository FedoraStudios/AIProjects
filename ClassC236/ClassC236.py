...
try: sys.path.append(glob.glob('../carla/dist/carla-*%d.%d-%s.egg' %(
...
def generate_radar_blueprint(blueprint_library):
	...
	radar_blueprint.set_attribute('point_per_second', str(15000))
...
try:
	...
	get_blueprint_world = world.get_blueprint_library()
	car_model = get_blueprint_world.filter('model3')[0]
	spawn_point = world.get_map().get_spawn_points()[6]
	dropped_vehicle = world.spawn_actor(car_model, spawn_point)

	mustang_car_blueprint = get_blueprint_world.filter('mustang')[0]
	mustang_spawn_point = world.get_map().get_spawn_points()[4]
    mustang_car = world.spawn_actor(mustang_car_blueprint, mustang_spawn_point)
    ...
    #Radar Sensor
    radar_sensor = generate_radar_blueprint(get_blueprint_world)
    ...
    def _Radar_callback(radar_data):
    	...
    	for detection in radar_data:
    		...
    		print('Distance: ', distance_name_data['distance'])

    actor_list(radar_data)
    time.sleep(1000)

finally:
	print('Destroying Actos...')
