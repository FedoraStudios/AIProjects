...
def semantic_lidar_data(point_cloud_data):
    ...
    for detection in point_cloud_data:
        ...
        Current_car_predicton = model.predict(Current_car_data)
        predicton = int(Current_car_predicton[0])
        if distance_name_data == 'Vehicles':

            #Getting prediction
            if prediction == 1:
                dropped_vehicle.apply_control(carla.VehicleControl(throttle = 1, reverse = True))
                dropped_vehicle.set_light_state(carla.VehicleLightState.Reverse)

                break             

            else:
                dropped_vehicle.apply_control(carla.VehicleControl(throttle = 0.5))
                dropped_vehicle.set_light_state(carla.VehicleLightState.LowBeam)
