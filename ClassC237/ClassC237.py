...
throttle = 0.5
saved_model_file = open('classifer_model.pkl', 'rb')
...
try:
	...
	#initializing carla blueprint library
    get_blueprint_of_world = world.get_blueprint_library()
    car_model = get_blueprint_of_world.filter('model3')[0]
    spawn_point = world.get_map().get_spawn_point()[6]
    dropped_vehicle = world.spawn_actor(car_model, spawn_point)

    #Mustang
    mustang_car_blueprint = get_blueprint_of_world.filter('mustang')[0]
    mustang_car_spawn_point = world.get_map().get_spawn_point()[4]
    ...
    #applying control to dropped vehicle
    dropped_vehicle.apply_control(carla.VehicleControl(throttle = 0.5))

    #Simulator Camera
    simulator_camera_location_rotation = carla.Transform(spawn_point.location, spawn_point.rotation)
    simulator_camera_location_rotation.location += spawn.point.get_forward_vector()*30
    ...
    sensor_radar_spawn_point = carla.Transform(carla.Location(x = -1.0, z = 1.8))
    ...
    def _Radar_callback(radar_data):
    	...
    	for detection in radar_data:
    		...
    		predicton = int(Current_car_prediction[0])
    		...
    		if predicton == 1:
    			...
    			print('Your model took the decision to go left since the prediction is equal to one')
    			...
    			print('Your model took the decision to go right since the prediction is equal to zero')
    			...
    			print('Your model took the decision to go straight since the prediction is equal to two')
